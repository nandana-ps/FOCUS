...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function showDashboard() {
    ...
    const { data: usage } = await supabase.from("usage_logs").select("*").eq("user_id", USER_ID).eq("date", today).single();
  ...
  function renderUsage(usage, limit) {
    ...
    list.innerHTML = "";
    const sites = [
      { name: "YouTube", used: usage.youtube_mins || 0, limit },
      { name: "Instagram", used: usage.instagram_mins || 0, limit }
    ];
    sites.forEach((site) => {
