...
${suffix}`;
  ...
  var SUPABASE_URL = "https://qzpgigcwgudmijtrzmrl.supabase.co";
  var SUPABASE_ANON_KEY = "sb_publishable_Tdv8n-7UhNbjuo-71kxM_Q_c3mbcIK4";
  var supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
  var USER_ID = "00000000-0000-0000-0000-000000000001";
  ...
  async function init() {
    ...
    const { data, error } = await supabase.from("profiles").select("settings").eq("id", USER_ID).single();
    if (error) {
  ...
  async function syncWithSupabase() {
    ...
    const { data: existing } = await supabase.from("usage_logs").select("*").eq("user_id", USER_ID).eq("date", today).single();
    let updates = {
      user_id: USER_ID,
      date: today,
    ...
    const { error } = await supabase.from("usage_logs").upsert(updates, { onConflict: "user_id, date" });
    if (!error) {
  ...
  async function checkLimits(siteName) {
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const { data: profile } = await supabase.from("profiles").select("settings").eq("id", USER_ID).single();
    const siteLimit = profile?.settings?.normal_limit || 60;
    limits[siteName] = { limit: siteLimit };
    const { data: usage } = await supabase.from("usage_logs").select("youtube_mins, instagram_mins").eq("user_id", USER_ID).eq("date", today).single();
    const serverUsage = usage ? siteName === "YouTube" ? usage.youtube_mins : usage.instagram_mins : 0;
    ...
    if (totalUsed >= siteLimit) {
      console.log(`FocusLock: \u{1F6AB} BLOCKING ${siteName}!`);
